...
void Fuzzy10Bin::FindMembershipValueForTriangles(double Input, double* Triangles, double* MembershipFunctionToSave, int TrianglesLength)
{
	int Temp = 0;

	for (int i = 0; i < TrianglesLength; i += 4) 
	{
...
void Fuzzy10Bin::LOM_Defazzificator(FuzzyRules* Rules, double* Input1, double* Input2, double* Input3, double* ResultTable, int RulesLength)
{
	...
	for (int i = 0; i < RulesLength; i++) 
	{
		...
		if ((Input1[Rules[i].Input1] > 0) && (Input2[Rules[i].Input2] > 0) && (Input3[Rules[i].Input3] > 0))
		{
			...
			double Min = 0;
			Min = m_min(Input1[Rules[i].Input1], m_min(Input2[Rules[i].Input2], Input3[Rules[i].Input3], double_less), double_less);
...
void Fuzzy10Bin::ApplyFilter(double Hue, double Saturation, double Value, int Method, double *ReturnValue)
{
	...
	FindMembershipValueForTriangles(Hue, HueMembershipValues, HueActivation, 32);		//输出HueActivation, 数组中八个数之和为1
	...
	FindMembershipValueForTriangles(Value, ValueMembershipValues, ValueActivation, 12);		//输出ValueActivation, 数组中两个数之和为1
